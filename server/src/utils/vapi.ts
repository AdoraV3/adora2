...
export class VapiService {
  ...
  async purchasePhoneNumber(phoneNumber: string): Promise<string> {
    try {
      ...
      console.log("[v0] Extracting area code from", phoneNumber, "-> areaCode:", areaCode)
      ...
      throw new Error(
        `Failed to purchase phone number ${phoneNumber}: ${error.response?.data?.message || error.message}`,
      )
    }
  }
...
assistantId: string, name: string, voiceId: string, p0?: string,
    // systemPrompt?: string,
